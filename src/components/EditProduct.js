    ...
    return (
        <div style={{ width: "50%", margin: "auto" }}>
            <div className="row m-2 p-3">
                <p className="mt-5 font-weight-bold mb-1">Изменить: </p>
                <form style={{ width: "100%" }}>
                    ...
                    <textarea className="form-control" id="price" value={data.price || "" }
                    onChange={(e)=>GetValueInputForm(e)}></textarea>
                    <label className="mt-3">Статус</label>
                    <select className="form-control" id="status" value={data.status }
                    onChange={(e)=>GetValueInputForm(e)}>
                        <option defaultValue={data.status}>Выполнено</option>
